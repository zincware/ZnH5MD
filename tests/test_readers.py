...
# pytest.mark.parametrize("atoms_list", [atoms_list, diff_atoms_list], indirect=True)
...
def test_diff_atoms_list(tmp_path, diff_atoms_list):
    os.chdir(tmp_path)
    db = znh5md.io.DataWriter(filename="db.h5")
    db.initialize_database_groups()

    reader = znh5md.io.AtomsReader(diff_atoms_list, frames_per_chunk=10)
    db.add(reader)
